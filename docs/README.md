...
> `data/Top_N_Faculty_{year}.xlsx` is auto-generated by the pipeline after Step 1 — no need to download it.

**3. Set up virtual environment**

- Set up the virtual environment by running the following code:
...
- **For each new terminal session:** run `source venv/bin/activate` from the project root before running any pipeline commands.
...
- Each user registers their own free account.
  ...
  1. Subscribe to the **Web of Science Starter API**
  1. Once approved, copy your API key from the application details page
...
- Point `FACULTY_FULL_PATH` to the new faculty list file in `code/config.py`:
